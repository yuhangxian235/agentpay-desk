...
- x402 facilitator adapter boundary with optional HTTP POST client and settlement receipt metadata.
...
- x402 facilitator settlement falls back locally unless `X402_FACILITATOR_URL` is configured and reachable.
...
3. Real x402 integration
   ...
   - Wire `x402Facilitator.ts` to a real facilitator endpoint and store facilitator responses.
...
```text
The demo now has real HTTP API boundaries, a facilitator adapter with an HTTP client path, server-side merchant ops state, a replaceable storage adapter, audit trail, browser E2E, CI, and live smoke checks. The remaining work to make it a real payment product is replacing the demo repository and simulated signer with production database storage and real wallet/facilitator settlement.
```
